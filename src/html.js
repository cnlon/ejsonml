...
export default function installToHtml (JsonML, document) {
  /**
   * Attribute name map
   *
   * @private
  ...
  var patch = JsonML.patch = function (elem, jml, filter) {
    for (var i = 1; i < jml.length; i++) {
      if (isArray(jml[i]) || typeof jml[i] === 'string') {
        // append children
        appendDOM(elem, toHTML(jml[i], filter))
      } else if (isMarkup(jml[i])) {
        appendDOM(elem, toDOM(jml[i].value))
      } else if (typeof jml[i] === 'object' && jml[i] !== null && elem.nodeType === 1) {
        // add attributes
  ...
  var toHTML = JsonML.toHTML = function (jml, filter) {
    try {
      ...
      if (!isArray(jml) || (typeof jml[0]) !== 'string') {
